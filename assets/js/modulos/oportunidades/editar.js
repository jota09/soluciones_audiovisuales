window.onload = function () {
   $('#form_cont #cuenta_id').select2('readonly', false);
   $('#form_cont #asesor_id').select2('readonly', false);
};
$(document).ready(function () {
   $('#cuenta_id').select2('readonly', false);
   $('#form_cont #cuenta_id').select2('readonly', false);
   $('#oportunidad_id').select2('readonly', true);
   ...
   $(' #asesor_asignado_id').select2('readonly', false);


   $("#toma_contacto_id").change(function () {

      if ($("#toma_contacto_id").val() == 11) {
         $("#campanna_id").parent().parent().parent().removeClass('hidden');
      } else {
         $("#campanna_id").parent().parent().parent().addClass('hidden');
      }
   ...
   bootbox.dialog({
      ...
      buttons: {
         success: {
            ...
            callback: function () {
               $.ajax({
                  url: 'index.php?modulo=oportunidades&accion=guardarActividad&ajax=true',
                  type: 'POST',
                  dataType: 'json',
                  async: false,
