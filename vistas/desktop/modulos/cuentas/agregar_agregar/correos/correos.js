function agregarCorreo() {
    var html = '<div  class="col-lg-12 form-group">' +
            ...
            '<div class="col-lg-6">' +
            '<input type="text" class="form-control validate[required]"  id="barrio" name="e_mail[]" >' +
            '</div>' +
            '<div class=" col-lg-2">' +
            '<input type="radio" class="form-control chk_correo_principal" name="principal">' +
            '</div>' +
            '<div class="col-lg-1">' +
            '<button type="button"  class="btn btn_eliminar_reg_correo btn-primary"><i class="fa fa-minus"></i></button> ' +
            '</div>' +
            '</div>';

    $('#div_agregar_correo').append(html);
    triggersCorreo();
}
...
$(document).ready(function () {
    $('#btn_agregar_correo').click(function () {
        agregarCorreo();
    });
    triggersCorreo();
});
