...
class S3Upload {
    ...
    /**
     * Asigna las extensiones permitidas, con ello se asigna directamente el mime/type.
    ...
    /**
     * Carga el tamaño maximo de archivo en bytes desde el archivo config.yml 
     * 
     * @global object $aplicacion
     */
    ...
    /**
     * Carga todos los mime/type disponibles en un array con su respectiva extension
     */
    ...
    /**
     * Funcion para verificar si hay filtro por extension o no
     * 
     * @return boolean
     */
    ...
    /**
     * Funcion final para subir archivo
     * 
     * @param string $var -> nombre del input del archivo que cargará
     * @param boolean $hash -> Si el nombre debe generar un aleatorio
     * @param boolean/int $arr -> si se envio las imagenes como array, se indica en que posicion
     * @return array -> Mensajes de error y/o success
     */
    public function subirArchivo($var, $hash = true, $arr = false) {
        if (isset($_FILES[$var]) && count($_FILES[$var]) > 0) {
            $destino = substr($this->carpeta, -1, 1) != '/' ? $this->carpeta . '/' : $this->carpeta;

            if ($arr !== false && is_int($arr)) {
                if (isset($_FILES[$var]['name'][$arr])) {
                    $tipo = $_FILES[$var]['type'][$arr];
                    $peso = $_FILES[$var]['syze'][$arr];
                    $nombre_tmp = $_FILES[$var]['tmp_name'][$arr];
                    $nombre = $_FILES[$var]['name'][$arr];
                } else {
                    $this->mensajes['errores'][] = array('codigo' => '404', 'titulo' => 'Index no existe', 'mensaje' => 'El indice ' . $arr . ', no se encuentro en el array: ' . $var);
                    $nombre = '';
                }
            } else {
    ...
    private function generarNombre($nombre_archivo) {
        ...
        $nombre = $hash . '-' . base64_encode($aplicacion->getUsuario()->getNombreCompleto()) . '-' . preg_replace("/ /", "_", $nombre_archivo);
        return $nombre;
    }

    public function limpiarMensajes() {
        $this->mensajes = array('errores' => array(), 'success' => array('cargado' => 0, 'mensaje' => '', 'url' => ''));
    }
