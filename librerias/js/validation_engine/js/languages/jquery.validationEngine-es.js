...
//                    "regex": /^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i,
            ...
            "ajaxAccount": {
               "url": "index.php?modulo=clientes&accion=comprorbarDuplicado",
               // you may want to pass extra data on the ajax call
               "extraDataDynamic": ['#propietario_naturaleza_id', '#propietario_nombre', '#propietario_apellido'],
               "alertTextLoad": "* Verificando, espere por favor",
               "alertText": "* Este Cliente ya se encuentra registrado"
            },
            "ajaxCase": {
               "url": "index.php?module=Cases&action=comprorbar_duplicado",
               // you may want to pass extra data on the ajax call
               "extraDataDynamic": ['#account_id', '#canal_esc', '#tipo_esc', '#record'],
               "alertTextLoad": "* Verificando, espere por favor",
               "alertText": "* Este Caso ya esta registrado con un cliente,Negocio, Escalamiento y tipificación Similar"
            },
            "ajaxNameCall": {
            ...
            "ajaxValidarDireccion": {
               "url": "index.php?modulo=captaciones&accion=validarDirecion",
               "alertText": "* Esta dirección se encuentra en uso, completala o cambiala!",
               "alertTextLoad": "* Cargando, espere por favor"
            },
            "validate2fields": {
            ...
            "comparacionFechas": {
               "alertText2": "* Requerid@ ",
               "alertText": " Rango inválido "
            },
            "comparacionFechaActual": {
               "alertText": "* Es inferior a la fecha Actual "
            },
            "comparacionFechaActualMayor": {
               "alertText": "* Es Inferior a la fecha "
            },
            "comparacionFechaActualMenor": {
               "alertText": "* Es Superior a la fecha "
            },
            "validaMIME": {
               "alertText": "* Tipo de Archivo NO Válido"
            },
            "comparacionContrasenias": {
               "alertText": "* Las contraseñas no son iguales"
            },
            "verificarProduct": {
               "alertText": "* Este producto no se puede agregar dos veces"
            },
         };
...
// Agregar el Input class="validate[funcCall[compararFechas[#fecha_creacion_desde]]]"
function compararFechas(field, rules, i, options) {
...
function comparaConFechaActual(field, rules, i, options) {
   ...
   var input = '#' + field.context.id; //obj que hace la peticion
   var hoy = fechaActual();
   var fecha = $(input).val(); // fecha del obj de peticion

//console.log($(input).html());
   for (var j = 0; j < options.InvalidFields.length; j++) {
      //console.log(options.InvalidFields);
      //console.log(options.InvalidFields[j].id + ' ID === VAL ' +options.InvalidFields[j].value + ' === ' + j);
      if (options.InvalidFields[j].id === field.context.id) {
         var fecha = options.InvalidFields[j].value;
      }
   }
   if (fecha !== '') {
      ...
      if (rules[6] !== "''") {
         ...
         var fI = fecha.split('-');
         var fF = $(rules[6]).val().split('-');

         var f1 = new Date(fI[0], fI[1], fI[2]); //31 de diciembre de 2015
         var f2 = new Date(fF[0], fF[1], fF[2]); //30 de noviembre de 2014
         //console.log("conparacion " + f1 + " < " + f2);
         if (f1 < f2) {
            return options.allrules.comparacionFechaActualMayor.alertText + ' (' + $(rules[6]).val() + ') ';
         }

      } else if (rules[7] !== "''") {

         var fI = fecha.split('-');
         var fF = $(rules[7]).val().split('-');

         var f1 = new Date(fI[0], fI[1], fI[2]); //31 de diciembre de 2015
         var f2 = new Date(fF[0], fF[1], fF[2]); //30 de noviembre de 2014
         //console.log("conparacion " + f1 + " > " + f2);
         if (f1 > f2) {
            return options.allrules.comparacionFechaActualMenor.alertText + ' (' + $(rules[7]).val() + ') ';
         }
      }
...
function validarContrasenias(field, rules, i, options) {
   if (s3vars.registro.id > 0) {
      if ($("#contrasenia").val() !== "" || "" !== $("#contrasenia2").val()) {

         if ($("#contrasenia").val().length > 7) {
            letras = /[A-Z]/;
            if (letras.test($("#contrasenia").val())) {
               numero = /[0-9]/;
               if (numero.test($("#contrasenia").val())) {
                  if ($("#contrasenia").val() === $("#contrasenia2").val()) {
                     return true;
                  } else {
                     return options.allrules.comparacionContrasenias.alertText;
                  }
               } else {
                  return options.allrules.minUnNumero.alertText;
               }
            } else {
               return options.allrules.minUnaMayus.alertText;
            }
         } else {
            return options.allrules.minSize.alertText + " 8 Caracteres";
         }
...
// Para Usarla validate[funcCall[verificarProduct]]
function verificarProduct(field, rules, i, options) {

   var aux = 0, fielVal = $(field).attr('data-product-id');
   if (fielVal !== '') {
      $.each($('.product-id'), function (k, v) {
         var currentVal = $(v).attr('data-product-id');
         if (fielVal.toUpperCase() === currentVal.toUpperCase()) {
            aux++;
         }
      });
      ...
      if (aux <= 1) {
         ...
         return options.allrules.verificarProduct.alertText;
      }
   }
}
